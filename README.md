...
> **💡 PCでもスマホでも利用できます。**  
> どちらのデバイスでも初めて開いたときに、APIキー取得と基本設定のウィザードが自動で表示されます。  
> APIキーは一度取得しておけば、どのデバイスでも同じキーを設定して使えます。
...
スマホで使い始める際は、設定タブで以下を入力する必要があります：
...
#### スマホへのAPIキー入力方法（どちらか選択）

**方法A：メモアプリ経由（おすすめ）**
1. PCでAPIキーをコピー
2. メモアプリ（スマホのメモ・LINEのKeepなど）に貼り付けて保存
3. スマホでアプリを開き、設定タブのAPIキー欄にペースト

**方法B：スマホのGoogle AI Studioから直接コピー**
1. スマホで [aistudio.google.com](https://aistudio.google.com) にアクセス（同じGoogleアカウントでログイン）
2. 左メニュー「API キー」→ 既存のキーをコピー
3. アプリの設定タブのAPIキー欄にペースト

※ Google AI StudioはPC向けサイトのためスマホでは見づらいですが、コピーするだけなら対応できます。
...
- 本アプリが提供する食材ガイドおよびAIによるレシピ提案は参考情報です。
- NG食材・アレルギー除外食材を踏まえた提案の利用については**自己責任**でお願いします。
- お子さまのアレルギーや健康状態については、必ずかかりつけ医にご相談ください。
- 予防接種スケジュールは厚生労働省のガイドラインに基づいた目安です。詳細は医療機関にご確認ください。
